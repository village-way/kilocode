---
title: CLI
description: Kilo CLI Optionen und Befehle.
---
...
Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Kilo interagieren.
...
Starten Sie die Terminalbenutzeroberfläche Kilo.
...
Der Kilo CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Kilo verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden Kilo-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
opencode attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Kilo. Zum Beispiel:
...
Kilo wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `kilo auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/opencode/auth.json` gespeichert.

```bash
kilo auth login
```

Wenn Kilo startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Kilo sehen möchten.
...
Sie können auch eine Verbindung zu einer laufenden `kilo serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Kilo-Sitzungen an.
...
Starten Sie einen Headless-Kilo-Server mit einer Weboberfläche.
...
Deinstallieren Sie Kilo und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Kilo auf die neueste Version oder eine bestimmte Version.

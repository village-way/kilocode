...
Der Befehl `kilo serve` startet einen headless HTTP-Server.
...
Der Benutzername ist standardmaessig `opencode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `kilo serve` und `opencode web`.
...
Mit `kilo serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `kilo serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Kilo-[IDE](/docs/ide)-Plugins.

---
title: Web
description: Verwenden Sie Kilo in Ihrem Browser.
---

Kilo kann als Webanwendung in Ihrem Browser ausgeführt werden und bietet das gleiche leistungsstarke KI-Codierungserlebnis, ohne dass ein Terminal erforderlich ist.

![Kilo Web – Neue Sitzung](../../../assets/web/web-homepage-new-session.png)
...
Dadurch wird ein lokaler Server auf `127.0.0.1` mit einem zufällig verfügbaren Port gestartet und Kilo automatisch in Ihrem Standardbrowser geöffnet.
...
Standardmäßig wählt Kilo einen verfügbaren Port aus. Sie können einen Port angeben:
...
Standardmäßig bindet der Server an `127.0.0.1` (nur localhost). So machen Sie Kilo in Ihrem Netzwerk zugänglich:

```bash
opencode web --hostname 0.0.0.0
```

Bei Verwendung von `0.0.0.0` zeigt Kilo sowohl lokale als auch Netzwerkadressen an:

---
title: GitHub
description: Verwenden Sie Kilo in GitHub-Problemen und Pull-Requests.
---

Kilo lässt sich in Ihren GitHub-Workflow integrieren. Erwähnen Sie `/opencode` oder `/oc` in Ihrem Kommentar, und Kilo führt Aufgaben in Ihrem GitHub Actions-Runner aus.
...
- `model`: Das mit Kilo zu verwendende Modell. Nimmt das Format `provider/model` an. Dies ist **erforderlich**.
...
Führen Sie Kilo nach einem Zeitplan aus, um automatisierte Aufgaben auszuführen:
...
Wenn für `pull_request`-Ereignisse kein `prompt` bereitgestellt wird, überprüft Kilo standardmäßig die Pull-Anfrage.
...
Überschreiben Sie die Standardaufforderung, um das Verhalten von Kilo für Ihren Workflow anzupassen.
...
Hier sind einige Beispiele, wie Sie Kilo in GitHub verwenden können.
...
- **Erklären Sie ein Problem**
  ...
  Kilo liest den gesamten Thread, einschließlich aller Kommentare, und antwortet mit einer klaren Erklärung.
...
- **PRs überprüfen und Änderungen vornehmen**
  ...
  Kilo implementiert die angeforderte Änderung und schreibt sie an denselben PR fest.

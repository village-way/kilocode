---
title: Konfig
description: Verwenden Sie die Konfiguration Kilo JSON.
---

Sie können Kilo mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

Kilo unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Wenn Kilo gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`kilo.ai/config.json`**](https://kilo.ai/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `kilo serve` und `opencode web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres Kilo-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer Kilo-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht Kilo, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer Kilo-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift Kilo mit einer Warnung auf `"build"` zurück.
...
Kilo lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
Kilo erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert Kilo mit benutzerdefinierten Tools, Hooks und Integrationen.
...
Dies ist nützlich, wenn Sie Kilo darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.

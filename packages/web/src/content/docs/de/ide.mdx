---
title: IDE
description: Die Kilo-Erweiterung für VS Code, Cursor und andere IDEs
---

Kilo lässt sich in VS Code, Cursor oder jeden IDE integrieren, der ein Terminal unterstützt. Führen Sie einfach `opencode` im Terminal aus, um zu beginnen.

---

## Verwendung

- **Schnellstart**: Verwenden Sie `Cmd+Esc` (Mac) oder `Ctrl+Esc` (Windows/Linux), um Kilo in einer geteilten Terminalansicht zu öffnen, oder fokussieren Sie eine dieser Terminalsitzung, falls bereits eine ausgeführt wird.
- **Neue Sitzung**: Verwenden Sie `Cmd+Shift+Esc` (Mac) oder `Ctrl+Shift+Esc` (Windows/Linux), um eine neue Kilo-Terminalsitzung zu starten, auch wenn eine bereits geöffnet ist. Sie können auch auf die Kilo-Schaltfläche im UI klicken.
- **Kontextbewusstsein**: Teilen Sie Ihre aktuelle Auswahl oder Registerkarte automatisch mit Kilo.
...
So installieren Sie Kilo auf VS Code und beliebten Forks wie Cursor, Windsurf, VSCodium:
...
Suchen Sie im Extension Marketplace nach **Kilo** und klicken Sie auf **Installieren**.

---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Kilo.
---

import config from "../../../../config.mjs"
export const console = config.console

Kilo verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Kilo-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Kilo anpassen
config.
...
OpenCode Zen ist eine Liste von Modellen, die vom Kilo-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Kilo funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Kilo aus und gehen Sie zu [kilo.ai/auth](https://kilo.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Kilo und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit Kilo:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Kilo ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Kilo wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Kilo ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Kilo-Konfiguration hinzufügen.
...
um das Token im Kilo-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[compliance note]
Kilo verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Kilo
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Kilo:
...
So verwenden Sie Google Vertex AI mit Kilo:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Kilo ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Kilo konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Kilo-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Kilo-Konversation als separate Sitzung aufgeführt.
...
Sie können Kilo für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Kilo über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Kilo über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Kilo konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit Kilo:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Kilo.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Kilo verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom Kilo-Team bereitgestellt werden. [Learn more](/docs/zen).
...
3. Geben Sie Ihren Kilo API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Kilo-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Kilo-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Kilo:
...
Sie können Modelle auch über Ihre Kilo-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Kilo verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann Kilo erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `kilo auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Kilo-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Kilo-Konfiguration übereinstimmt.

...
Kilo verwendet die `permission`-Konfiguration, um zu entscheiden, ob eine bestimmte Aktion automatisch ausgeführt werden soll, Sie dazu auffordern oder blockiert werden soll.
...
Verwenden Sie `external_directory`, um Toolaufrufe zuzulassen, die Pfade außerhalb des Arbeitsverzeichnisses berühren, in dem Kilo gestartet wurde. Dies gilt für jedes Werkzeug, das einen Pfad als Eingabe verwendet (z. B. `read`, `edit`, `list`, `glob`, `grep` und viele `bash`-Befehle).
...
Kilo permissions are keyed by tool name, plus a couple of safety guards:
...
Wenn Sie nichts angeben, beginnt Kilo mit zulässigen Standardeinstellungen:
...
Wenn Kilo zur Genehmigung auffordert, bietet UI drei Ergebnisse:

- `once` — approve just this request
- `always` – zukünftige Anfragen genehmigen, die den vorgeschlagenen Mustern entsprechen (für den Rest der aktuellen Kilo-Sitzung)

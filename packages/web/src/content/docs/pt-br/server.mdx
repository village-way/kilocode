...
O comando `kilo serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente opencode pode usar.
...
```bash
kilo serve [--port <número>] [--hostname <string>] [--cors <origem>]
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opencode`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `kilo serve` quanto ao `opencode web`.

```bash
OPENCODE_SERVER_PASSWORD=sua-senha kilo serve
```
...
Você pode executar `kilo serve` para iniciar um servidor autônomo. Se você tiver o TUI do opencode em execução, `kilo serve` iniciará um novo servidor.

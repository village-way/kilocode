...
O arquivo de configuração tem um esquema que está definido em [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Você pode configurar as configurações do servidor para os comandos `kilo serve` e `opencode web` através da opção `server`.

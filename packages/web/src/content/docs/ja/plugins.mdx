---
title: プラグイン
description: Kilo を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Kilo を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Kilo のデフォルトの動作を変更したりできます。
...
Kilo は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
:::note
Kilo デスクトップ アプリを使用している場合は、応答の準備ができたとき、またはセッション エラーが発生したときにシステム通知を自動的に送信できます。
:::
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://kilo.ai/docs/sdk) を参照してください。

---
title: IDE
description: VS Code、Cursor、およびその他の IDE 用の Kilo 拡張機能
---

Kilo は、VS Code、Cursor、またはterminal をサポートする任意の IDE と統合します。開始するには、terminal で `opencode` を実行するだけです。

---

## 使用法

- **クイック起動**: `Cmd+Esc` (Mac) または `Ctrl+Esc` (Windows/Linux) を使用して、分割terminal ビューで Kilo を開くか、既存のterminal セッションが既に実行されている場合はそれにフォーカスします。
- **新しいセッション**: すでに開いている場合でも、`Cmd+Shift+Esc` (Mac) または `Ctrl+Shift+Esc` (Windows/Linux) を使用して、新しい Kilo terminal セッションを開始します。 UI の [Kilo] ボタンをクリックすることもできます。
- **コンテキスト認識**: 現在の選択またはタブを Kilo と自動的に共有します。
...
Extension Marketplace で **Kilo** を検索し、**インストール** をクリックします。

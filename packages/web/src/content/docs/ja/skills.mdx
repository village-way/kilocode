...
エージェント スキルにより、Kilo はリポジトリまたはホーム ディレクトリから再利用可能な命令を検出できます。
...
プロジェクトのローカル パスの場合、Kilo は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
...
Kilo では、`skill` ツールの説明に利用可能なスキルがリストされています。

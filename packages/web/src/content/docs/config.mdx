---
title: Config
description: Using the Kilo JSON config.
---

You can configure Kilo using a JSON config file.

---

## Format

Kilo supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
When Kilo starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
The config file has a schema that's defined in [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
You can configure server settings for the `kilo serve` and `opencode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Kilo server.
...
You can configure the providers and models you want to use in your Kilo config through the `provider`, `model` and `small_model` options.
...
You can configure the theme you want to use in your Kilo config through the `theme` option.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Kilo will fall back to `"build"` with a warning.
...
Kilo will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend Kilo with custom tools, hooks, and integrations.
...
This is useful when you want to restrict Kilo to only use specific providers rather than disabling them one by one.

...
[**Kilo**](/)는 오픈 소스 AI 코딩 에이전트입니다. terminal 기반 인터페이스, 데스크탑 앱 또는 IDE 확장으로 사용할 수 있습니다.
...
당신의 terminal에 있는 Kilo를 사용하려면, 당신은 필요로 할 것입니다:
...
Kilo를 설치하는 가장 쉬운 방법은 설치 스크립트를 통해 입니다.
...
> 최신 릴리스를 위해 Kilo 탭을 사용하는 것이 좋습니다. 공식 `brew install opencode` 공식은 Homebrew 팀에 의해 유지되고 더 자주 업데이트됩니다.
...
:::tip[추천: WSL 사용]
Windows에서 최고의 경험을 위해 [Windows Subsystem for Linux (WSL)](/docs/windows-wsl)를 사용하는 것이 좋습니다. Kilo의 기능으로 더 나은 성능과 전체 호환성을 제공합니다.
:::
...
Bun을 사용하여 Windows에서 Kilo 설치 지원은 현재 진행 중입니다.

[Releases](https://github.com/Kilo-Org/kilo/releases)에서 이진을 할 수도 있습니다.

---

## 구성

Kilo를 사용하면 API 키를 구성하여 LLM 공급자를 사용할 수 있습니다.

LLM 공급자를 사용하는 새로운 경우, [OpenCode Zen](/docs/zen)를 사용하는 것이 좋습니다.
Kilo에 의해 테스트 및 확인 된 모델의 큐레이터 목록입니다.
팀.

1. TUI에서 `/connect` 명령을 실행하고, opencode를 선택하고, [kilo.ai/auth](https://kilo.ai/auth)에 머리를 선택합니다.
...
다음, 다음 명령을 실행하여 프로젝트의 Kilo를 초기화합니다.

```bash frame="none"
/init
```

Kilo를 사용하여 프로젝트를 분석하고 `AGENTS.md` 파일을 만들 수 있습니다.
프로젝트 루트.
...
이 도움말 Kilo는 프로젝트 구조와 코딩 패턴을 이해
사용.
...
Kilo를 사용하여 프로젝트에 작업할 준비가 되어 있습니다. 자주 묻는 질문
모두!
...
Codebase를 설명하기 위해 Kilo를 요청할 수 있습니다.
...
프로젝트에 새로운 기능을 추가하려면 Kilo를 요청할 수 있습니다. 우리는 먼저 계획을 만들 것을 묻는 것이 좋습니다.

1. **플랜을 선택 **

   Kilo는 Plan mode 로 변경할 수 있는 능력을 비활성화하고
   ...
   **Tab** 키를 사용하여 전환합니다. 오른쪽 하단에 있는 이 지표를 볼 수 있습니다.
   ...
   당신이 원하는 것을 이해하기 위해 Kilo를 충분히 세부 정보를 제공하려는. 그것은 도움
   팀의 주니어 개발자에게 이야기하고 싶습니다.

   :::tip
   Kilo를 많은 컨텍스트와 예제를 제공하여 당신이 무엇을 이해하는 데 도움이
   이름 \*
   :::
...
2. **플랜에 대해서 **
   ...
   Kilo는 어떤 이미지를 스캔할 수 있습니다. 당신은 할 수
...
더 똑바른 변화를 위해, 당신은 Kilo를 직접 그것을 건설할 수 있습니다
첫 플랜을 검토하지 않고.
...
Kilo는 이제 당신이 만든 변경을 반전하고 원래 메시지를 표시
다시.
...
여기에서 당신은 신속하고 다시 시도 Kilo를 요청할 수 있습니다.
...
여기 [example 대화](https://kilo.ai/s/4XP1fce5) 는 opencode 입니다.

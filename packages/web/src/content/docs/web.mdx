---
title: Web
description: Using Kilo in your browser.
---

Kilo can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![Kilo Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens Kilo in your default browser.
...
By default, Kilo picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make Kilo accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, Kilo will display both local and network addresses:
...
Once started, the web interface provides access to your Kilo sessions.

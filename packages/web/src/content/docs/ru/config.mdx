...
Файл конфигурации имеет схему, определенную в [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Вы можете настроить параметры сервера для команд `kilo serve` и `opencode web` с помощью опции `server`.

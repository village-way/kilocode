...
opencode jest dostępny przez listę dostawców w [Models.dev](https://models.dev), więc można użyć `kilo auth login` do skonfigurowania klucza API dla dowolnego dostawcy, który może być kontrolowany. Jest to możliwe w `~/.local/share/opencode/auth.json`.
...
Można również zastosować, uruchamiając `kilo serve`, aby wyłączyć zimne uruchamianie serwera MCP przy każdym uruchomieniu:

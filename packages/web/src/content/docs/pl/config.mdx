...
Plik konfiguracyjny ma schemat końcowy w [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Możliwość ustawienia serwera dla źródeł `kilo serve` i `opencode web` za pomocą opcji `server`.

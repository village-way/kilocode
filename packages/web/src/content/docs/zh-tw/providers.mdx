---
title: 提供商
description: 使用 Kilo 中的任何 LLM 提供商。
---

import config from "../../../../config.mjs"
export const console = config.console

Kilo 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev) 來支持 **75+ LLM 提供商**，並且它支持運行本地模型。
...
您可以在 Kilo 配置的 `provider` 部分自定義提供商。
...
OpenCode Zen 是 Kilo 團隊提供的模型列表，這些模型已被
經過測試和驗證，可以與 Kilo 良好配合。 [了解更多](/docs/zen)。

:::tip
如果您是新手，我們建議您從 OpenCode Zen 開始。
:::

1. 在 TUI 中運行 `/connect` 命令，選擇 opencode，然後前往 [kilo.ai/auth](https://kilo.ai/auth)。
...
它的工作方式與 Kilo 中的任何其他提供商一樣，並且完全可以選擇使用。
...
:::info
[Anthropic](https://anthropic.com) 不正式支持在 Kilo 中使用您的 Claude Pro/Max 訂閱。
:::
...
:::note
如果您不想將令牌存儲在 Kilo auth 存儲中，也可以指定 `GITLAB_TOKEN` 環境變量。
:::

##### Self-Hosted GitLab

:::note[compliance note]
Kilo 使用小型模型來執行某些 AI 任務，例如生成會話標題。
默認情況下，它配置為使用 gpt-5-nano，由 Zen 託管。若要鎖定 Kilo
...
2. Feature flags（通過 Rails console）：
...
要將 GitHub Copilot 訂閱與 Kilo 結合使用：
...
要將 Google Vertex AI 與 Kilo 結合使用：
...
如果您發現 Helicone 的功能或模型未通過 Kilo 自動配置，您始終可以自行配置。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能可讓您將相關的 LLM 請求分組在一起。使用 [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 插件自動將每個 Kilo 對話記錄為 Helicone 中的會話。
...
該插件將 `Helicone-Session-Id` 和 `Helicone-Session-Name` 標頭注入您的請求中。在 Helicone 的 Sessions 頁面中，您將看到每個 Kilo 對話都列為單獨的會話。
...
您可以通過 [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server 實用程序配置 Kilo 以使用本地模型
...
您可以通過 LM Studio 配置 Kilo 以使用本地模型。
...
您可以通過 Ollama 配置 Kilo 以使用本地模型。

:::tip
Ollama 可以自動為 Kilo 配置自身。詳情請參閱 [Ollama integration docs](https://docs.ollama.com/integrations/opencode)。
:::
...
6. **Important**：在 Kilo 中使用雲模型之前，您必須將模型信息拉取到本地：
...
OpenCode Zen 是 Kilo 團隊提供的經過測試和驗證的模型列表。 [了解更多](/docs/zen)。
...
5. 您還可以通過 Kilo 配置自定義它們。這是指定提供商的示例
...
要將 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) 與 Kilo 一起使用：
...
您還可以通過 Kilo 配置自定義模型。以下是指定提供者路由順序的示例。
...
:::tip
您可以將任何與 OpenAI 兼容的提供商與 Kilo 結合使用。大多數現代 AI 提供商都提供與 OpenAI 兼容的 API。
:::
...
`limit` 字段允許 Kilo 了解您還剩下多少上下文。標準提供商會自動從 models.dev 中提取這些內容。
...
1. **Check the auth setup**：運行 `kilo auth list` 以查看憑據是否
   提供商的配置已添加到您的配置中。

   這不適用於 Amazon Bedrock 等依賴環境變量進行身份驗證的提供商。

2. 對於自定義提供商，請檢查 Kilo 配置並：
   - 確保 `/connect` 命令中使用的提供商 ID 與 Kilo 配置中的 ID 匹配。

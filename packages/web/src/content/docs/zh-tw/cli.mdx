...
默認情況下，Kilo CLI 在不帶任何參數運行時啟動[TUI](/docs/tui)。

```bash
opencode
```

但它也接受本頁記錄的命令。這允許您以編程方式與 Kilo 交互。
...
將 terminal 連接到通過 `serve` 或 `web` 命令啟動的已運行的 Kilo 後端服務器。

```bash
opencode attach [url]
```

這允許將 TUI 與遠程 Kilo 後端一起使用。例如：
...
Kilo 由 [Models.dev](https://models.dev) 上的提供商列表提供支持，因此您可以使用 `kilo auth login` 為您想要使用的任何提供商配置 API 密鑰。它存儲在`~/.local/share/opencode/auth.json` 中。

```bash
kilo auth login
```

當 Kilo 啟動時，它會從憑據文件加載提供商。如果您的環境中定義了任何鍵或項目中的 `.env` 文件。
...
使用`--refresh`標誌來更新緩存的模型列表。當新模型已添加到提供商並且您希望在 Kilo 中查看它們時，這非常有用。
...
您還可以附加到正在運行的 `kilo serve` 實例，以避免每次運行時 MCP 服務器冷啟動時間：

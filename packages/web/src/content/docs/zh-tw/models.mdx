...
默認情況下會預加載大多數流行的提供商。如果您已通過 `/connect` 命令添加了提供商的憑據，那麼它們將在您啟動 Kilo 時可用。
...
以下是與 Kilo 配合良好的幾個模型，排名不分先後。 （這不是詳盡的列表，也不一定是最新的）：
...
許多型號支持具有不同配置的多種變體。Kilo 附帶了流行提供商的內置默認變體。

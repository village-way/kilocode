...
Možete dodati vanjske alate u Kilo koristeći _Model Context Protocol_, ili MCP. Kilo podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Kilo Config](https://kilo.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Kilo automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Kilo će:
...
Ako server zahtijeva autentifikaciju, Kilo će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Kilo sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Kilo, zajedno s ugrađenim alatima. Možete njima upravljati kroz Kilo konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Kilo sa vašim Sentry nalogom.

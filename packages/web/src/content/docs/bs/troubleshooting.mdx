...
Da biste otklonili probleme s Kilo, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Kilo Desktop pokreće lokalni Kilo server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Kilo` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
Kilo također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Kilo mogao ponovo izgraditi.

1. Potpuno zatvorite Kilo Desktop.
...
3. Ponovo pokrenite Kilo Desktop.

---

### Popravite probleme sa vezom na serveru

Kilo Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, Kilo Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Kilo karakteristikama.

---

### Obavještenja se ne prikazuju

Kilo Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Kilo u postavkama vašeg OS-a, i
...
1. Zatvorite Kilo Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Kilo Desktop aplikacije):

---
title: CLI
description: Kilo CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Kilo CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.
...
Pokrenite korisnički interfejs Kilo terminala.
...
Kada se Kilo pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Kilo.
...
Također možete priključiti pokrenutu `kilo serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| `--attach`   |        | Priključite na pokrenuti kilo server (npr. http://localhost:4096)    |
...
Pokrenite Kilo server bez glave sa web interfejsom.
...
Kilo se može konfigurirati pomoću varijabli okruženja.

---
title: Config
description: Korištenje Kilo JSON konfiguracije.
---

Možete konfigurirati Kilo koristeći JSON konfiguracijski fajl.

---

## Format

Kilo podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
:::
Kada se Kilo pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Možete konfigurirati postavke servera za komande `kilo serve` i `opencode web` putem opcije `server`.
...
- `mdns` - Omogući otkrivanje mDNS usluge. Ovo omogućava drugim uređajima na mreži da otkriju vaš Kilo server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Kilo konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Kilo pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Možete konfigurirati temu koju želite koristiti u svojoj Kilo konfiguraciji putem opcije `theme`.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je subagent, Kilo će se vratiti na `"build"` s upozorenjem.
...
Kilo će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
[Plugins](/docs/plugins) proširuju Kilo sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite Kilo da koristi samo određene provajdere umesto da ih onemogućavate jednog po jednog.
:::note
`disabled_providers` ima prioritet nad `enabled_providers`.
:::

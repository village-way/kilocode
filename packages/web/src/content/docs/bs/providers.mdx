---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Kilo.
---

import config from "../../../../config.mjs"
export const console = config.console

Kilo koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Kilo konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Kilo
config.
...
OpenCode Zen je lista modela koje je obezbedio Kilo tim koji su bili
testirano i potvrđeno da dobro radi sa Kilo. [Saznajte više](/docs/zen).
...
Da biste koristili Amazon Bedrock s Kilo:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Kilo.
:::
...
da pohrani token u kilo auth memoriju.
:::

##### Samostalni GitLab

:::note[compliance note]
Kilo koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Kilo
...
Za korištenje Google Vertex AI s Kilo:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Kilo razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Kilo razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za Kilo. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Kilo:
...
3. Kopirajte API ključ za korištenje u Kilo.
...
OpenCode Zen je lista testiranih i verifikovanih modela koju je obezbedio Kilo tim. [Saznajte više](/docs/zen).
...
Polja `limit` omogućavaju Kilo da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `kilo auth list` da vidite da li su vjerodajnice

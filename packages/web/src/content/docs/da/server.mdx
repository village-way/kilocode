...
Kommandoen `kilo serve` kjører en hodeløs HTTP-server som avslører et OpenAPI-endepunkt som en opencode-klient kan bruge.
...
Sett `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grunntilføjende autentisering. Brugernavnet er standard til `opencode`, eller sett `OPENCODE_SERVER_USERNAME` for at overstyre det. Dette gjelder både `kilo serve` og `opencode web`.
...
Du kan kjøre `kilo serve` for at starte en frittstående server. Hvis du har
opencode TUI kjører, vil `kilo serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan bruges til at kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet bruges av Kilo [IDE](/docs/ide) plugins.
...
Kilo-serveren viser følgende APIer.

---
title: Konfig
description: Ved at bruge Kilo JSON konfig.
---

Du kan konfigurere Kilo ved hjælp af en JSON-konfigurationsfil.

---

## Format

Kilo understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
Konfigurationsfilen har et skema, der defineres i [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `kilo serve` og `opencode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Kilo-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Kilo-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Kilo at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Kilo-konfiguration, gennem indstillingen `theme`.
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Kilo falde tilbage til `"build"` med en advarsel.
...
Kilo vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide Kilo med brugerdefinerede værktøjer, kroge og integrationer.
...
Dette er nyttigt, når du vil begrænse Kilo til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.

---
title: CLI
description: Kilo CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Kilo CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Kilo programmatisk.
...
Start Kilo terminalbrugergrænsefladen.
...
Tilslut en terminal til en allerede kørende Kilo backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Kilo backend. For eksempel:
...
Kilo drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `kilo auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
kilo auth login
```

Når Kilo starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Kilo.
...
Du kan også vedhæfte til en kørende `kilo serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
Start en hovedløs Kilo-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator Kilo sessionsholder.
...
Liste over alle Kilo sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Kilo-sessioner.
...
Importer sessionsdata fra en JSON fil eller Kilo del URL.
...
Start en hovedløs Kilo-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Kilo via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller Kilo og fjern alle relaterede filer.
...
Opdaterer Kilo til den nyeste version eller en specifik version.
...
Kilo kan konfigureres ved hjælp af miljøvariabler.

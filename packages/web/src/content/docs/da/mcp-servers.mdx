...
Du kan tilføje eksterne værktøjer til Kilo ved hjælp af _Model Context Protocol_ eller MCP. Kilo understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Kilo Config](https://kilo.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Kilo håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Kilo:
...
Hvis serveren kræver godkendelse, vil Kilo bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Kilo opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Kilo-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Kilo til din Sentry-konto.

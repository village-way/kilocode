---
title: Leverandører
description: Bruge en hvilken som helst LLM-leverandør i Kilo.
---

import config from "../../../../config.mjs"
export const console = config.console

Kilo bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for at støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.
...
testet og verifisert for at fungere godt med Kilo. [Finn ut mer](/docs/zen).
...
Det fungerer som alle andre leverandører i Kilo og er helt valgfritt at bruge.
...
Slik bruger du Amazon Bedrock med Kilo:
...
:::info
Å bruge Claude Pro/Max-abonnementet ditt i Kilo støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
for at lagre token i kilo auth-lagring.
:::

##### Selvhostet GitLab

:::note[compliance note]
Kilo bruger en liten modell for nogle AI oppgaver som at generere sessiontittelen.
Den er konfigurert til at bruge gpt-5-nano som standard, hostet av Zen. For at låse Kilo
...
Slik bruger du GitHub Copilot-abonnementet med Kilo:
...
Slik bruger du Google Vertex AI med Kilo:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Kilo-samtale som en session i Helicone.
...
Programtiltilføjet injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Kilo-samtale oppført som en separat session.
...
Slik bruger du Ollama Cloud med Kilo:
...
3. Kopier API-nøgleen for brug i Kilo.
...
6. **Viktig**: Før du bruger skymodeller i Kilo, må du hente modellinformasjonen lokalt:
...
Slik bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Kilo:
...
1. **Tjek autentiseringsoppsettet**: Kjør `kilo auth list` for at se om legitimasjonen

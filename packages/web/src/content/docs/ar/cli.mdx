---
title: CLI
description: أوامر وخيارات CLI في Kilo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Kilo افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
opencode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Kilo برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Kilo.
...
يتضمن CLI الخاص بـ Kilo أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Kilo قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opencode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Kilo تعمل عن بعد. على سبيل المثال:
...
يعتمد Kilo على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `kilo auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opencode/auth.json`.

```bash
kilo auth login
```

عند تشغيل Kilo يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Kilo.
...
يمكنك أيضا الإرفاق بمثيل `kilo serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
بدء خادم Kilo بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Kilo لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Kilo.

```bash
opencode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Kilo.
...
بدء خادم Kilo بدون واجهة مع واجهة ويب.

```bash
opencode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Kilo عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
إلغاء تثبيت Kilo وإزالة جميع الملفات ذات الصلة.
...
يمكن تهيئة Kilo باستخدام متغيرات البيئة.

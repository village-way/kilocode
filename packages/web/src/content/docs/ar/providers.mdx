---
title: المزوّدون
description: استخدام أي موفّر LLM في Kilo.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Kilo ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Kilo.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Kilo.
...
OpenCode Zen هي قائمة نماذج يوفّرها فريق Kilo وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Kilo. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [kilo.ai/auth](https://kilo.ai/auth).
...
يعمل مثل أي مزوّد آخر في Kilo واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع Kilo:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Kilo غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم Kilo نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Kilo
...
لاستخدام Google Vertex AI مع Kilo:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Kilo تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Kilo مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Kilo. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Kilo.
...
6. **مهم**: قبل استخدام نماذج السحابة في Kilo، يجب سحب معلومات النموذج محليا:
...
OpenCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Kilo. [اعرف المزيد](/docs/zen).
...
3. أدخل مفتاح API الخاص بـ Kilo.
...
تسمح حقول `limit` لـ Kilo بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `kilo auth list` لمعرفة ما إذا كانت بيانات الاعتماد

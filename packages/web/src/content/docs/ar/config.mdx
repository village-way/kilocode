---
title: الإعدادات
description: استخدام ملف إعدادات Kilo بصيغة JSON.
---

يمكنك ضبط Kilo باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم Kilo تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
عند تشغيل Kilo، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `kilo serve` و`opencode web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم Kilo.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات Kilo عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول Kilo استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات Kilo عبر الخيار `theme`.
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود Kilo إلى `"build"` مع تحذير.
...
سيقوم Kilo بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
[الإضافات](/docs/plugins) توسّع Kilo بأدوات وخُطافات وتكاملات مخصصة.
...
هذا مفيد عندما تريد تقييد Kilo لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.

...
opencode 由 [Models.dev](https://models.dev) 上的模型列表提供支持，因此您可以使用 `kilo auth login` 来为您想要使用的任何模型配置 API 按键。它存储在 `~/.local/share/opencode/auth.json` 中。
...
您还可以添加到正在运行的`kilo serve`实例，会员每次运行时MCP服务器冷启动时间：

...
`kilo serve` komutu, opencode istemcisinin kullanabilecegi bir OpenAPI endpoint'i acan headless bir HTTP sunucusu calistirir.
...
Sunucuyu HTTP basic auth ile korumak icin `OPENCODE_SERVER_PASSWORD` ayarlayin. Kullanici adi varsayilan olarak `opencode` degeridir; degistirmek isterseniz `OPENCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `kilo serve` hem de `opencode web` icin gecerlidir.
...
`kilo serve` ile bagimsiz bir sunucu baslatabilirsiniz. opencode TUI aciksa, `kilo serve` yeni bir sunucu baslatir.

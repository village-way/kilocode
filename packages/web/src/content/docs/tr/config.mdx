...
Yapılandırma dosyası [**`kilo.ai/config.json`**](https://kilo.ai/config.json)'da tanımlanan bir şemaya sahiptir.
...
`kilo serve` ve `opencode web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.

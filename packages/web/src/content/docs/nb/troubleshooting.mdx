...
For å feilsøke problemer med Kilo, start med å sjekke loggene og lokale data den lagrer på disken.
...
Kilo lagrer øktdata og andre applikasjonsdata på disken på:
...
Kilo Desktop kjører en lokal Kilo-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `Kilo`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
Kilo kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at Kilo kan gjenoppbygge den.

1. Avslutt Kilo Desktop helt.
...
3. Start Kilo Desktop på nytt.

---

### Løs problemer med servertilkobling

Kilo Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
På Windows krever Kilo Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til Kilo.

---

### Varsler vises ikke

Kilo Desktop viser bare systemvarsler når:

- varsler er aktivert for Kilo i OS-innstillingene dine, og
...
1. Avslutt Kilo Desktop.
2. Finn og slett disse filene (de finnes i Kilo Desktop-appens datakatalog):
...
Hvis du opplever problemer med Kilo:
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte leverandørpakker. Kilo installerer dynamisk leverandørpakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start Kilo på nytt for å installere de nyeste leverandørpakkene på nytt

Dette vil tvinge Kilo til å laste ned de nyeste versjonene av leverandørpakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
Kilo vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavleverktøy i rekkefølgen: `xclip` og `xsel`.

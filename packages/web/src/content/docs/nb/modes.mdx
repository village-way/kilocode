...
:::caution
Modi er nå konfigurert gjennom alternativet `agent` i Kilo-konfigurasjonen. De
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Kilo lar deg tilpasse oppførselen, verktøyene og ledetekstene for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Kilo-konfigurasjonen.
...
Kilo kommer med to innebygde moduser.
...
Hvis ingen temperatur er spesifisert, bruker Kilo modellspesifikke standarder (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale Kilo-konfigurasjonen og den prosjektspesifikke konfigurasjonen.

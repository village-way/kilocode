...
Kilo CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
opencode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Kilo programmatisk.
...
Start Kilo-terminalbrukergrensesnittet.
...
Kilo CLI har også følgende kommandoer.

---

### agent

Administrer agenter for Kilo.
...
Koble en terminal til en allerede kjørende Kilo-backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Kilo-backend. For eksempel:
...
Kilo drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `kilo auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/opencode/auth.json`.

```bash
kilo auth login
```

Når Kilo starter opp, laster den leverandørene fra legitimasjonsfilen. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Kilo.
...
Kjør Kilo i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `kilo serve`-forekomst for å unngå MCP-tjenerkaldoppstartstider ved hver kjøring:
...
| `--attach`   |      | Koble til en kjørende Kilo-server (f.eks. http://localhost:4096)     |
...
Start en hodeløs Kilo-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
kilo serve
```

Dette starter en HTTP-server som gir API tilgang til Kilo-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Vis tokenbruk og kostnadsstatistikk for Kilo-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Kilo share-URL.

```bash
opencode import <file>
```

Du kan importere fra en lokal fil eller en Kilo share-URL.
...
Start en hodeløs Kilo-server med et webgrensesnitt.

```bash
opencode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Kilo via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Avinstaller Kilo og fjern alle relaterte filer.
...
Oppdaterer Kilo til siste versjon eller en spesifikk versjon.
...
Kilo CLI bruker følgende globale flagg.
...
Kilo kan konfigureres ved hjelp av miljøvariabler.

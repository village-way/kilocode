...
Du kan konfigurere Kilo ved å bruke en JSON konfigurasjonsfil.

---

## Format

Kilo støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
Når Kilo starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Konfigurasjonsfilen har et skjema som er definert i [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `kilo serve` og `opencode web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Kilo-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Kilo-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Kilo å bruke en billigere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan konfigurere temaet du vil bruke i Kilo-konfigurasjonen gjennom alternativet `theme`.
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Kilo falle tilbake til `"build"` med en advarsel.
...
Kilo vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Kilo alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Kilo med tilpassede verktøy, kroker og integrasjoner.
...
Dette er nyttig når du vil begrense Kilo til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.

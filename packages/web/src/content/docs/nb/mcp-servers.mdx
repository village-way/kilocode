...
Du kan legge til eksterne verktøy i Kilo ved å bruke _Model Context Protocol_ eller MCP. Kilo støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Kilo Config](https://kilo.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du ber om LLM.
...
Kilo håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Kilo:
...
Hvis serveren krever autentisering, vil Kilo be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Kilo lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Kilo, sammen med innebygde verktøy. Så du kan administrere dem gjennom Kilo-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Kilo til Sentry-kontoen din.

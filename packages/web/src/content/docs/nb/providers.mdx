---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Kilo.
---

import config from "../../../../config.mjs"
export const console = config.console

Kilo bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøklene for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Kilo-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Kilo
konfig.
...
OpenCode Zen er en liste over modeller levert av Kilo-teamet som har vært
testet og verifisert for å fungere godt med Kilo. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg opencode og gå til [kilo.ai/auth](https://kilo.ai/auth).
...
Det fungerer som alle andre leverandører i Kilo og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med Kilo:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Kilo:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Kilo skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Kilo-konfigurasjonen.
...
for å lagre token i Kilo auth-lagring.
:::

##### Selvhostet GitLab

:::note[compliance note]
Kilo bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Kilo
...
Slik bruker du GitHub Copilot-abonnementet med Kilo:
...
Slik bruker du Google Vertex AI med Kilo:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Kilo, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Kilo-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Kilo-samtale oppført som en separat økt.
...
Du kan konfigurere Kilo for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Kilo for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Kilo for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Kilo. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Kilo:
...
3. Kopier API-nøkkelen for bruk i Kilo.
...
6. **Viktig**: Før du bruker skymodeller i Kilo, må du hente modellinformasjonen lokalt:
...
OpenCode Zen er en liste over testede og verifiserte modeller levert av Kilo-teamet. [Finn ut mer](/docs/zen).
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Kilo-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Kilo-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Kilo:
...
Du kan også tilpasse modeller gjennom Kilo-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Kilo. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar Kilo forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `kilo auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Kilo-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Kilo-konfigurasjonen.

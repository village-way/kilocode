---
title: Regler
description: Set custom instructions for Kilo.
---

Du kan gi tilpassede instruksjoner for å Kilo ved å opprette en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for å tilpasse oppførselen til ditt spesifikke prosjekt.

---

## Initialiser

For å lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i Kilo.

:::tip
Du bør overgi prosjektets `AGENTS.md`-fil til Git.
:::

Dette vil skanne prosjektet og alt dets innhold for å forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper Kilo å navigere i prosjektet bedre.
...
Kilo støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/opencode/AGENTS.md`-fil. Dette blir brukt på alle Kilo-økter.
...
For brukere som migrerer fra Claude Code, støtter Kilo Claude Codes filkonvensjoner som fallbacks:
...
Når Kilo starter, ser den etter regelfiler i denne rekkefølgen:
...
Selv om Kilo ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:
...
Du kan lære Kilo å lese eksterne filer ved å gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørg for at Kilo laster filer kun når det er nødvendig for den spesifikke oppgaven

...
Kilo formaterer automatisk filer etter at de er skrevet eller redigert ved hjelp av språkspesifikke formatere. Dette sikrer at koden som genereres følger kodestilene til prosjektet ditt.

---

## Innebygd

Kilo kommer med flere innebygde formatere for populære språk og rammeverk. Nedenfor er en liste over formatere, støttede filutvidelser og kommandoer eller konfigurasjonsalternativer den trenger.
...
Så hvis prosjektet ditt har `prettier` i `package.json`, vil Kilo automatisk bruke det.

---

## Slik fungerer det

Når Kilo skriver eller redigerer en fil, gjør den:
...
Du kan tilpasse formatere gjennom `formatter`-delen i Kilo-konfigurasjonen.

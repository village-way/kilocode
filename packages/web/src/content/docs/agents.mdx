...
There are two types of agents in Kilo; primary agents and subagents.
...
Kilo comes with four built-in primary agents: **Build**, **Plan**, **Debug**, and **Orchestrator**. We'll
look at these below.
...
Kilo comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Kilo comes with four built-in primary agents and two built-in subagents.
...
A debugging-focused agent that uses the same tools as Build but follows a systematic diagnostic methodology. It will reflect on multiple possible causes, narrow them down, add logging to validate assumptions, and ask you to confirm before applying a fix.

Use this when you're troubleshooting issues, investigating errors, or diagnosing problems. You can disable it if you prefer using Build for everything:
...
A coordination agent that breaks down complex tasks and delegates them to specialized subagents (`explore` and `general`) in parallel. The orchestrator has read-only access to the codebase and cannot edit files directly — all implementation work is delegated to subagents via the Task tool.

Use this for complex, multi-step projects that require coordination across different areas of the codebase. You can disable it if you don't need it:
...
If no temperature is specified, Kilo uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Kilo config and the project specific config.
...
- **Debug agent**: Systematic debugging with diagnostic methodology — reflects on causes, adds logging, confirms before fixing
- **Orchestrator agent**: Breaks down complex tasks and delegates to subagents in parallel

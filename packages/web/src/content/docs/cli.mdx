...
But it also accepts commands as documented on this page. This allows you to interact with Kilo programmatically.
...
Start the Kilo terminal user interface.
...
Attach a terminal to an already running Kilo backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote Kilo backend. For example:
...
Kilo is powered by the provider list at [Models.dev](https://models.dev), so you can use `kilo auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
kilo auth login
```

When Kilo starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Kilo.
...
You can also attach to a running `kilo serve` instance to avoid MCP server cold boot times on every run:
...
Start a headless Kilo server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
Show token usage and cost statistics for your Kilo sessions.
...
Import session data from a JSON file or Kilo share URL.

```bash
opencode import <file>
```

You can import from a local file or an Kilo share URL.
...
Start a headless Kilo server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access Kilo through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Kilo can be configured using environment variables.

...
You can add external tools to Kilo using the _Model Context Protocol_, or MCP. Kilo supports both local and remote servers.
...
You can define MCP servers in your [Kilo Config](https://kilo.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Kilo automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Kilo will:
...
If the server requires authentication, Kilo will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Kilo will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Kilo, alongside built-in tools. So you can manage them through the Kilo config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Kilo to your Sentry account.

---
title: IDE
description: L'estensione Kilo per VS Code, Cursor e altri IDE
---

Kilo si integra con VS Code, Cursor o qualunque IDE che supporti un terminal. Basta eseguire `opencode` nel terminal per iniziare.

---

## Utilizzo

- **Avvio rapido**: usa `Cmd+Esc` (Mac) o `Ctrl+Esc` (Windows/Linux) per aprire Kilo in una vista terminal divisa, oppure per mettere a fuoco una sessione esistente se e gia in esecuzione.
- **Nuova sessione**: usa `Cmd+Shift+Esc` (Mac) o `Ctrl+Shift+Esc` (Windows/Linux) per avviare una nuova sessione di Kilo, anche se ne esiste gia una aperta. Puoi anche cliccare il pulsante Kilo nell'interfaccia.
- **Consapevolezza del contesto**: condividi automaticamente con Kilo la selezione corrente o la scheda attiva.
...
Per installare Kilo su VS Code e fork popolari come Cursor, Windsurf, VSCodium:
...
Cerca **Kilo** nel Marketplace delle estensioni e clicca **Install**.

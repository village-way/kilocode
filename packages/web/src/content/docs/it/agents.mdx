...
In Kilo esistono due tipi di agenti: agenti primari e subagenti.
...
Kilo include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Kilo include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Kilo include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, Kilo usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path e' relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Kilo sia per la config specifica del progetto.
...
L'ID modello nella configurazione Kilo usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCode Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/Kilo-Org/kilo).
:::

---
title: Supporto ACP
description: Usa Kilo in qualunque editor compatibile con ACP.
---

Kilo supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Kilo via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia Kilo come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Kilo' nel selettore agenti di AI Chat.
...
Per usare Kilo come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Kilo come agente ACP per la chat.
...
Kilo funziona via ACP come nel terminal. Tutte le funzionalita sono supportate:
...
- MCP servers configured in your Kilo config

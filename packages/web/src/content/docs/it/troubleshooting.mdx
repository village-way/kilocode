...
Per diagnosticare problemi con Kilo, inizia controllando i log e i dati locali che salva su disco.
...
Kilo Desktop esegue in background un server locale di Kilo (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Kilo` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
Kilo puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Kilo possa ricostruirla.

1. Quit Kilo Desktop completely.
...
3. Restart Kilo Desktop.

---

### Risolvi problemi di connessione al server

Kilo Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, Kilo Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminal su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Kilo.

---

### Notifiche non visualizzate

Kilo Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Kilo nelle impostazioni del sistema operativo, e
...
1. Quit Kilo Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Kilo Desktop):
...
Se riscontri problemi con Kilo:
...
3. Assicurati di avere l'ultima versione con `kilo upgrade`

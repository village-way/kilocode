...
Puoi aggiungere strumenti esterni a Kilo usando il _Model Context Protocol_, o MCP. Kilo supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Kilo Config](https://kilo.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Kilo gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Kilo:
...
Se il server richiede autenticazione, Kilo ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Kilo memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Kilo insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Kilo come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Kilo al tuo account Sentry.

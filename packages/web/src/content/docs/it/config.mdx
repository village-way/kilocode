---
title: Config
description: Usa la config JSON di Kilo.
---

Puoi configurare Kilo usando un file di configurazione JSON.

---

## Formato

Kilo supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Quando Kilo si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Puoi configurare le impostazioni del server per i comandi `kilo serve` e `opencode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Kilo.
...
Puoi configurare provider e modelli da usare in Kilo tramite le opzioni `provider`, `model` e `small_model`.
...
Puoi configurare il tema da usare in Kilo tramite l'opzione `theme`.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Kilo fara fallback su `"build"` mostrando un avviso.
...
Kilo scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono Kilo con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare Kilo a usare solo alcuni provider invece di disabilitarli uno per uno.

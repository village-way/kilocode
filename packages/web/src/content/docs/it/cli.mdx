---
title: CLI
description: Opzioni e comandi della CLI di Kilo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Kilo, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Kilo in modo programmatico.
...
Avvia l'interfaccia testuale di Kilo (terminal user interface).
...
La CLI di Kilo include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Kilo.
...
Collega un terminal a un backend server di Kilo gia' in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend Kilo remoto. Per esempio:
...
Kilo si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `kilo auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
kilo auth login
```

Quando Kilo si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. E' utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Kilo.
...
Puoi anche collegarti a una istanza `kilo serve` gia' in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
Avvia un server Kilo headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.
...
Mostra statistiche di utilizzo token e costo per le sessioni Kilo.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Kilo.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione Kilo.
...
Avvia un server Kilo headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a Kilo tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla Kilo e rimuove tutti i file correlati.
...
Kilo puo' essere configurato tramite variabili d'ambiente.

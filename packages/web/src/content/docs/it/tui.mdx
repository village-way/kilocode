---
title: TUI
description: Usa l'interfaccia utente da terminal (TUI) di Kilo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Kilo fornisce un'interfaccia interattiva da terminal (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Kilo avvia la TUI per la directory corrente.
...
Quando usi la TUI di Kilo, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Kilo. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Puoi personalizzare il comportamento della TUI tramite il file di config di Kilo.

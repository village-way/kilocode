---
title: configuración
description: Usando la configuración Kilo JSON.
---

Puede configurar Kilo usando un archivo de configuración JSON.

---

## Formato

Kilo admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Cuando se inicia Kilo, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
El archivo de configuración tiene un esquema definido en [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Puede configurar los ajustes del servidor para los comandos `kilo serve` y `opencode web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor Kilo.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración Kilo a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, Kilo intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración Kilo a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, Kilo recurrirá a `"build"` con una advertencia.
...
Kilo descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían Kilo con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir Kilo para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.

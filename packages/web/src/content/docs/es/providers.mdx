---
title: Proveedores
description: Usando cualquier proveedor LLM en Kilo.
---

import config from "../../../../config.mjs"
export const console = config.console

Kilo usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Kilo.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Kilo
configuración.
...
OpenCode Zen es una lista de modelos proporcionados por el equipo Kilo que han sido
probado y verificado para funcionar bien con Kilo. [Más información](/docs/zen).
...
Funciona como cualquier otro proveedor en Kilo y su uso es completamente opcional.
...
Para usar Amazon Bedrock con Kilo:
...
:::info
El uso de su suscripción Claude Pro/Max en Kilo no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Kilo utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Kilo
...
Para utilizar Google Vertex AI con Kilo:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Kilo como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Kilo enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para Kilo. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Kilo:
...
3. Copie la clave API para usarla en Kilo.
...
6. **Importante**: Antes de usar modelos en la nube en Kilo, debe obtener la información del modelo localmente:
...
OpenCode Zen es una lista de modelos probados y verificados proporcionada por el equipo Kilo. [Más información](/docs/zen).
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Kilo:
...
Los campos `limit` le permiten a Kilo comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `kilo auth list` para ver si las credenciales

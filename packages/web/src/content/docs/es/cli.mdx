---
title: CLI
description: Kilo CLI opciones y comandos.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

El Kilo CLI de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Kilo mediante programación.
...
Inicie la interfaz de usuario del terminal Kilo.
...
El Kilo CLI también tiene los siguientes comandos.
...
Esto permite usar TUI con un backend remoto Kilo. Por ejemplo:
...
Cuando se inicia Kilo, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Kilo.
...
También puede conectarse a una instancia `kilo serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
Puede importar desde un archivo local o una URL compartida Kilo.
...
Inicie un servidor Kilo sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Kilo a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).

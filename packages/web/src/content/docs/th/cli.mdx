---
title: CLI
description: Kilo CLI รับสัญญาณและคำสั่ง
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Kilo CLI โดยเฉพาะอย่างยิ่งจะเริ่มต้น [TUI](/docs/tui) ไม่ต้องทำงานโดยไม่มีข้อใดใดๆ

```bash
opencode
```

การยอมรับคำสั่งตามเอกสารในนั้นด้วยเหตุนี้ทำให้คุณสามารถพบกับ Kilo โดยทางโปรแกรมได้
...
เริ่มเปิดให้ผู้ใช้เริ่มใช้ Kilo
...
Kilo CLI ยังมีคำสั่งอีกมากมาย

---

### agent

จัดการตัวแทนสำหรับ Kilo
...
สาเหตุว่าทำไมเซิร์ฟเวอร์แบ็กเอนด์ Kilo ที่รันจริงๆ ซึ่งเริ่มต้นผ่านคำสั่ง `serve` หรือ `web`

```bash
opencode attach [url]
```

วิธีการใช้ TUI กับแบ็กเอนด์ Kilo ตัวอย่าง:
...
Kilo มีประสิทธิภาพสูงโดยรายชื่อทั่วไปที่ [Models.dev](https://models.dev) ดังนั้นเหตุผลที่เป็นไปได้ API สำหรับความต้องการใดๆ จำเป็นต้องใช้ข้อมูลนี้ถูกที่ `~/.local/share/opencode/auth.json`

```bash
kilo auth login
```

เมื่อ Kilo เริ่มต้นระบบจะโหลดจากไฟล์ข้อมูลรับรองและไม่จำเป็นต้องคำนึงถึงสิ่งใด ๆ ที่จะพบไฟล์ของคุณหรือ `.env` ในโปรเจ็กต์ของคุณ
...
ใช้แฟล็ก `--refresh` ในที่นี้รายการโมเดลที่แคชไว้ตรงนั้นมีประโยชน์เพิ่มโมเดลใหม่ให้กับระบบและขอดูโมเดลของข้อมูลใน Kilo
...
ขอแนบไปกับฟีเจอร์นี้ `kilo serve` ทำงานอยู่อีกครั้งเวลาของแขก MCP การทำงานหลายอย่าง:
...
เริ่มต้นเลานจ์ Kilo ไม่เพียงแต่สำหรับนักท่องเที่ยวเท่านั้น API ภาพถ่าย [เอกสารรับรอง](/docs/server) เพื่อดู HTTP เต็มรูปแบบ
...
แสดงสถิติการเริ่มต้นและค่าใช้จ่ายสำหรับ Kilo ของคุณ
...
นำเข้าข้อมูลที่ดีจากไฟล์ JSON หรือแชร์ Kilo URL

```bash
opencode import <file>
```

ไม่เคยนำเข้าไฟล์ในเครื่องหรือแชร์ Kilo URL
...
เชิญแขก Kilo อย่างเป็นทางการด้วยเว็บอินเตอร์เฟส

```bash
opencode web
```

ตัวควบคุมจะเริ่มต้นเซิร์ฟเวอร์ HTTP และเปิดเว็บเองเพื่อเข้าถึง Kilo ผ่านทางเว็บอินเตอร์เฟส จัดเตรียม `OPENCODE_SERVER_PASSWORD` สำหรับคำอธิบายคุณสมบัติความถูกต้องพื้นฐาน HTTP (ตัวยึดของฮาร์ดแวร์คือ `opencode`)
...
ไม่ว่า Kilo และลบไฟล์ทั้งหมดทั้งหมด
...
Kilo สามารถกำหนดค่าโดยใช้ตัวแปรสภาพแวดล้อม

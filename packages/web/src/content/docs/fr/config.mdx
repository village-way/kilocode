---
title: Configuration
description: En utilisant la configuration Kilo JSON.
---

Vous pouvez configurer Kilo à l'aide d'un fichier de configuration JSON.

---

## Format

Kilo prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
Lorsque Kilo démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Le fichier de configuration a un schéma défini dans [**`kilo.ai/config.json`**](https://kilo.ai/config.json).
...
Vous pouvez configurer les paramètres du serveur pour les commandes `kilo serve` et `opencode web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Kilo.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Kilo via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Kilo essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration Kilo via l'option `theme`.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Kilo reviendra à `"build"` avec un avertissement.
...
Kilo téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent Kilo avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre Kilo à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.

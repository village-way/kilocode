---
title: Web
description: En utilisant Kilo dans votre navigateur.
---

Kilo peut s'exécuter comme une application Web dans votre navigateur, offrant la même expérience puissante de codage d'IA sans avoir besoin d'un terminal.

![Kilo Web - Nouvelle session](../../../assets/web/web-homepage-new-session.png)
...
Cela démarre un serveur local sur `127.0.0.1` avec un port disponible aléatoire et ouvre automatiquement Kilo dans votre navigateur par défaut.
...
Par défaut, Kilo sélectionne un port disponible. Vous pouvez spécifier un port :
...
Par défaut, le serveur se lie à `127.0.0.1` (localhost uniquement). Pour rendre Kilo accessible sur votre réseau :

```bash
opencode web --hostname 0.0.0.0
```

Lorsque vous utilisez `0.0.0.0`, Kilo affichera les adresses locales et réseau :
...
Une fois démarrée, l'interface web permet d'accéder à vos sessions Kilo.
...
![Kilo Web - Session active](../../../assets/web/web-homepage-active-session.png)

### Statut du serveur

Cliquez sur "Voir les serveurs" pour afficher les serveurs connectés et leur état.

![Kilo Web - Voir Serveurs](../../../assets/web/web-homepage-see-servers.png)

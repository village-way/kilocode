---
title: CLI
description: Options et commandes CLI Kilo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI Kilo démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
opencode
```

Mais il accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec Kilo par programme.
...
Démarrez l’interface utilisateur du terminal Kilo.
...
La CLI Kilo dispose également des commandes suivantes.

---

### agent

Gérer les agents pour Kilo.
...
Cela permet d'utiliser le TUI avec un backend Kilo distant. Par exemple:
...
Lorsque Kilo démarre, il charge les fournisseurs à partir du fichier d'informations d'identification. Et s'il y a des clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'indicateur `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans Kilo.
...
Vous pouvez également vous connecter à une instance `kilo serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
Vous pouvez importer à partir d'un fichier local ou d'un partage Kilo URL.
...
Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à Kilo via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Kilo peut être configuré à l'aide de variables d'environnement.

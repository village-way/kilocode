...
description: Utilisez Kilo dans n’importe quel éditeur compatible ACP.
---

Kilo prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) ou (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser Kilo via ACP, configurez votre éditeur pour exécuter la commande `opencode acp`.

La commande démarre Kilo en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC via stdio.
...
Pour l'ouvrir, utilisez le nouvel agent « Kilo » dans le sélecteur d'agent AI Chat.
...
Pour utiliser Kilo comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser Kilo comme agent ACP pour le chat.
...
Kilo fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration Kilo

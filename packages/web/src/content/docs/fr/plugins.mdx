---
title: Plugins
description: Écrivez vos propres plugins pour étendre Kilo.
---

Les plugins vous permettent d'étendre Kilo en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Kilo.
...
Kilo exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
:::note
Si vous utilisez l'application de bureau Kilo, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://kilo.ai/docs/sdk) pour plus de détails.

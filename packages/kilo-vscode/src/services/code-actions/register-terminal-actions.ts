...
export function registerTerminalActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("zhanlu.new.terminalAddToContext", async (args: any) => {
    ...
    vscode.commands.registerCommand("zhanlu.new.terminalFixCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = await getTerminalContents(1)
      }
    ...
    vscode.commands.registerCommand("zhanlu.new.terminalExplainCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = await getTerminalContents(1)
      }

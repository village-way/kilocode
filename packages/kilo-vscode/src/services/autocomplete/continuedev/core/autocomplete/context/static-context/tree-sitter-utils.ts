...
export function findEnclosingTypeDeclaration(
  ...
  while (node && !["type_alias_declaration", "interface_declaration", "enum_declaration"].includes(node.type)) {
...
export function unwrapToBaseType(node: SyntaxNode): SyntaxNode {
  ...
  for (const child of node.namedChildren) {
    ...
    if (
      unwrapped !== child ||
      ["function_type", "tuple_type", "type_identifier", "predefined_type"].includes(unwrapped.type)
    ) {
      return unwrapped
    }
  }

...
class OpenRouter extends OpenAI {
  static override providerName = "openrouter"
  ...
  protected override extraBodyProperties(): Record<string, any> {
    if (this.completionOptions.reasoning === false) {
      return {
        reasoning: {
          effort: "minimal",
        },
  ...
  protected override modifyChatBody(body: ChatCompletionCreateParams): ChatCompletionCreateParams {
    ...
    const shouldCacheConversation = this.cacheBehavior?.cacheConversation || this.completionOptions.promptCaching
    const shouldCacheSystemMessage = this.cacheBehavior?.cacheSystemMessage || this.completionOptions.promptCaching
    ...
    const filteredMessages = body.messages.filter((m: any) => m.role !== "system" && !!m.content)

import { ContentBlockParam, MessageCreateParams, MessageParam } from "@anthropic-ai/sdk/resources"
...
const convertToAnthropic = (body: ChatCompletionCreateParams): ConversionResult => {
  ...
  const tools = body.tools?.filter((tool) => tool.type === "function").map((tool) => openaiToolToAnthropicTool(tool))
...
export const applyAnthropicCachingToOpenRouterBody = (
  ...
  const cachedSystem = Array.isArray(cachedBody.system) ? cachedBody.system : []
  ...
  systemMappings.forEach((mapping) => {
    ...
    if (Array.isArray(mapping.originalContent)) {
      ...
      slice.forEach((block: any, idx: number) => {
        ...
        if (originalIndex === null || originalIndex === undefined || !block?.cache_control) {
          return
        }
  ...
  messageMappings.forEach((mapping) => {
    ...
    const contentArray = Array.isArray(cachedMessage.content) ? cachedMessage.content : []

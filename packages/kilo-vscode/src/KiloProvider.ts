...
export class KiloProvider implements vscode.WebviewViewProvider {
   ...
   */
  private async handlePermissionResponse(permissionId: string, response: "once" | "always" | "reject"): Promise<void> {
    ...
    try {
      const workspaceDir = this.getWorkspaceDirectory()
      await this.httpClient.respondToPermission(this.currentSession.id, permissionId, response, workspaceDir)
    } catch (error) {

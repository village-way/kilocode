...
| Phase 4: Advanced (rest)     | ⬚ Not started | —          | Tooltips (4.1), DropdownMenu + ContextMenu (4.2), Toast (4.3), List for SessionList (4.4) still pending.                                                                                                                                                                            |
...
12. **`chat.css` reduced to 397 lines** (from 552 in the plan's baseline): Permission dialog CSS (~7 classes), tool/reasoning CSS (~150 lines), popover CSS (~85 lines) removed. Settings tab inline styles (~80 lines) eliminated. Remaining CSS covers layout (chat-view, prompt-input, session-list, model-selector popover internals).
